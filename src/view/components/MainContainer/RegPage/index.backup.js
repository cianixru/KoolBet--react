...
import { registrationAPI } from "config/constants";
...
class RegPage extends React.Component {
            ...
            .then(response => {
                if (response !== undefined) {
                    this.setState(prevState => {
                        if (prevState.signUpFieldsList !== response) {
                            // console.log('object :', response.response);
                            return { signUpFieldsList: response.response }
                        }
                    })
                }
            })
    ...
    handleSubmit = (e) => {
        console.log('this.state.response :', this.state.signUpFieldsList);
        e.preventDefault();
    ...
    render() {
        ...
        // let vols = Object.values(this.state.signUpFieldsList).map(e => e.name);
        // console.log(vols);
        
        let labelContainers = [];

        let vols = this.state.signUpFieldsList.map(e => 
            <li><b>{e.name}</b> -- {e.type} -- {e.hidden.toString()} - {e.sequence} - {e.readonly.toString()} - {e.mandatory.toString()}</li>
        );
        ...
        let inputList = this.state.signUpFieldsList.map(e => 
            {
                if(e.name=== "email") {
                    return (
                        <FormItem prefixCls="inputSection">
                            {getFieldDecorator('email', {
                                rules: [{
                                    ...
                                    required: e.mandatory, message: 'Please input your E-mail!',
                                }],
                            })(
                                <Input name={e.name} type={e.name}  placeholder="Your email address" />
                            )}
                        </FormItem>
                    );
            }
                            ...
                            <ul className="">
                                ...
                                <li>
                                    <div className="checkbox__blk">
                                        ...
                                        <label htmlFor="acceptTerms">I confirm that I am over 18 years old.</label>
                                    </div>
                                </li>
