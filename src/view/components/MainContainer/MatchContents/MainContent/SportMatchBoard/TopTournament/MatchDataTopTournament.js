...
class MatchDataTopTournament extends PureComponent {
    ...
    getTournamentsMatchs = () => {
        let matchs = [];
        this.props.inds.forEach((e) => {
            matchs = matchs.concat(this.props.state.currentTournamentData[e].tournament.value)
        })
        return matchs
    }

    render() {
        let matchs = this.getTournamentsMatchs()
        let prevDate = '';
        return (
            <section>
                            ...
                            <div className="row subhead">
                                <div className="cell-s match-start-time-value">
                                    <div><FormattedMessage
                                        id="MainContainer.MatchContents.MainContent.SportMatchBoard.MatchData.Column.Time"
                                        defaultMessage="TIME"/></div>
                                </div>
                                <div className="cell-s match-team-value">
                                    <div><FormattedMessage
                                        id="MainContainer.MatchContents.MainContent.SportMatchBoard.MatchData.Column.TableEvents"
                                        defaultMessage="TABLE EVENTS"/></div>
                                </div>
                                <div className="cell-a match-specialoddvalue-value">
                                    <div>
                                        {this.state.markets
                                        ...
                                        return <Fragment key={index}>
                                            {
                                                (new Date(tournament.startdate).toLocaleDateString() !== prevDate)
                                                    ...
                                                    <div className="row date">
                                                        <div
                                                            className="date-value">{prevDate = new Date(tournament.startdate).toLocaleDateString()}</div>
                                                    </div>
                                                    : null
                                            }
                                            ...
                                            <div className="row">
                                                <div className="cell-s match-start-time-value">
                                                    <div>{new Date(tournament.startdate).toLocaleTimeString([], {
                                                        hour: '2-digit',
                                                        minute: '2-digit'
                                                    })}</div>
                                                </div>
                                                ...
                                                <div className="cell-s go-to-all-markets"
                                                     data-toggle="toggle011 go-to-all-markets" id="go-to-all-markets"
                                                     data-toggler=".active">
                                                    <div className="go-to-all-markets__button">
                                                        <Link
                                                            to={`/sport/details/${tournament.sportId}-${tournament.countryId}-${tournament.tournamentId}-${tournament.matchId}`}>
                                                            +{tournament.groups.reduce((a, b) => {
...
export default connect(mapStateToProps)(withRouter(MatchDataTopTournament))